	...
	public static class MapGenerator
	{
		...
		public static void GenerateDungeonMap(StreamWriter writer, int seed, Rectangle area)
		{
			...
			while (nodes.Count != 0) {
				...
				int outX = (int)Math.Round(-sumX / (sumN * (0.5 + 1.5 * rnd.NextDouble())));
				outX = (outX > 1 ? 1 : outX); // HACK : Xna.Framework.MathHelper.Clamp is getting method access errors
				outX = (outX < -1 ? -1 : outX);

				int outY = (int)Math.Round(-sumY / (sumN * (0.5 + 1.5 * rnd.NextDouble())));
				outY = (outY > 1 ? 1 : outY);
				outY = (outY < -1 ? -1 : outY);
